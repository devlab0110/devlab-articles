...
Здравейте днес ще ви запозная с една моя идея и 
концепция за разработване на софтуер.

Ще се опитам да представя първо основната идея,
след това да я разширя и да разгледаме нещата в детайли.
...
семантичен начин така че взаимовръзките да се запазят.
...
---
Някои от вас ще си кажат: "Тука има много тафтология и повтаряне" ... да ама
така се запазват връзките между различните структори и това е много ключово.
...
Ако искате да смените името на конкретен атрибут от даден обект
... UI editor-а ще ви представи всички други места където се ползва
този обект и неговият атрибут.
...
нищо не пречи да имате отделни code builders за: api server, web app, mobile app и т.н.)
...
- може да се сложи история на мета обекта 
...
- може да се добави и feature които позволява четенето и скриването на конкретни env променливи 
...
тоест описанието се превръща в ядрото на проекта, 
...
на софтуер, а именно повторението и конфликтите на термини и понятия ... както и тяхното проследяване и взаимовръзки. 
...
Пуснали сте api сървър на Java ... утре решавате да пробвате Nodejs ... просто сменяте Code builder-а .. донастройвате Deploy tools и след малко ще имате същото api, но на Nodejs :D
